...
description: "Show modal on interaction, then wait for its submission — scoped to the same user and the modal's own custom-id — resolving to the ModalSubmitInteraction or…"
---

> **showAndAwaitModal**(`interaction`, `modal`, `options?`): `Promise`\<`ModalSubmitInteraction`\<`CacheType`\> \| `null`\>
...
ModalSubmitInteraction or `null` if the user dismisses it / it times
