...
Options for [gracefulShutdown](../functions/gracefulShutdown).
...
| <a id="property-onshutdown"></a> `onShutdown?` | (`signal`) => `Awaitable`\<`void`\> | Runs before `client.destroy()` — flush databases, close connections, etc. |
