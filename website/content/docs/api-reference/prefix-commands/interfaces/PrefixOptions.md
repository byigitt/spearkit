...
| <a id="property-dynamic"></a> `dynamic?` | (`message`) => `Awaitable`\<`string` \| readonly `string`[] \| `null` \| `undefined`\> | Resolve extra prefix(es) per message — e.g. a custom per-guild prefix from a database or createSettings. Returned prefixes are tried in addition to any static `prefix`. Return `null`/`undefined` for none. Keep it fast (and cached): it runs on every candidate message. |
