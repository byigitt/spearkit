...
Slash commands in spear are defined as a single object: the metadata, the typed
options, and the handler all live together. spear serialises them for discord
...
```ts
import { command, PermissionFlagsBits } from "spear";
...
and handler; spear routes to the right one automatically.
...
options. There is no `switch (subcommand)` to write — spear dispatches by the
...
```ts
import { CommandRegistry } from "spear";
...
If a handler throws, spear catches it. By default it emits the client's `error`
...
Commands must be registered with discord before they appear. spear gives you two
ways.
