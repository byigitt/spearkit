...
Discord localizes command names separately through `nameLocalizations` and
`descriptionLocalizations`. Runtime replies use the invoking user's
`interaction.locale`; prefix messages fall back to the guild's preferred locale.
spearkit's dependency-free `I18n` handles that response layer.
