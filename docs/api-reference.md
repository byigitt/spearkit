...
| `i18n` | `I18n \| undefined` | Runtime translations behind `ctx.t`. |
| `owners` | `readonly string[]` | Bot-owner ids for `requireBotOwner`. |
| `register(...items: Registerable[])` | `this` | Route each item to the matching registry. |
| `inviteUrl(options?)` | `string` | OAuth2 invite link (after ready). |
...
| `i18n` | `I18n \| I18nOptions` | Runtime catalogs, locale fallback, and optional async resolver. |
| `cooldownStore` | `CooldownBackend \| KeyValueStore` | Persist cooldowns across restarts and shards. |
| `owners` | `readonly string[]` | Bot-owner ids for `requireBotOwner()`. |
...
### Runtime i18n — [guide](./i18n.md)

```ts
type TranslationParam = string | number | boolean | Date | null | undefined;
type TranslationParams = Readonly<Record<string, TranslationParam>>;
type TranslationValue = string | ((params: TranslationParams) => string);
interface LocaleTarget { locale?; guildLocale?; guildId?; userId: string; }
interface I18nOptions {
  messages: Record<string, Record<string, TranslationValue>>;
  defaultLocale: string;
  fallbackLocale?: string;
  resolveLocale?: (target: LocaleTarget) => Awaitable<string | null | undefined>;
  missing?: (key: string, locale: string) => string;
}
class I18n<K extends string = string> {
  t(locale: string, key: K, params?: TranslationParams): string;
  localeFor(target: LocaleTarget): Promise<string>;
  translateFor(target: LocaleTarget, key: K, params?: TranslationParams): Promise<string>;
}
function createI18n<const M>(options: I18nOptions<M>): I18n<MessageKey<M>>;
// new SpearClient({ i18n }); await ctx.t("key", params)
```

### Everyday helpers — [guide](./dx.md)

```ts
function choices(map: Record<string, string | number>): OptionChoice[];
function choices(...values: string[]): OptionChoice[];
function filterChoices(items, query, options?: { limit?: number }): OptionChoice[];
function inviteUrl(options: { clientId; permissions?; scopes?; guildId?; disableGuildSelect? }): string;
function parseSnowflake/parseUserId/parseRoleId/parseChannelId(input: string): string | null;
function parseCustomEmoji(input: string): { name; id; animated } | null;
function slashMention(name, commandId, subcommand?): string;
// ctx.sendLong / ctx.dm / ctx.withTyping / ctx.progress / ctx.suggest
...
class CooldownManager { constructor(backend?: CooldownStoreInput); setBackend(backend); consume(bucket, input, actor, now?); peek(...); reset(...); clear(); }
class MemoryCooldownBackend implements CooldownBackend {}
interface CooldownBackend { hit(key, durationMs, now); peek(key, durationMs, now); delete(key); clear(); size?(); }
function keyValueCooldownBackend(store: KeyValueStore): CooldownBackend;
function redisCooldownBackend(client: RedisCommands, options?: { prefix?: string }): CooldownBackend;
type CooldownStoreInput = CooldownBackend | KeyValueStore;
...
// command({ cooldown }); new SpearClient({ cooldown, cooldownStore }); client.cooldowns
```

### Scaling — [guide](./scaling.md)

```ts
interface StartShardsOptions extends Omit<ShardingManagerOptions, "token"> {
  token?: string;
...
function shardListForWorker(totalShards: number, workerIndex: number, workerCount: number): number[];
function shardOptionsFromEnv(env?): Pick<ClientOptions, "shards" | "shardCount">;
function fetchShardStats(client: Client): Promise<ShardStatsReport>;

class WorkQueue {
  constructor(options?: { concurrency?: number; maxQueued?: number });
  readonly active: number; readonly queued: number; readonly pending: number;
...
function startHealthServer(options?: {
  ...
  host?: string;
  client?: Client;
  checks?: Record<string, () => Awaitable<boolean>>;
...
interface BatchUsageStore extends UsageStore { recordMany(events): Awaitable<void>; }
class BufferedUsageStore { constructor(downstream, { batchSize?, flushIntervalMs?, maxBuffered?, onDrop?, onError? }); record; all; flush; close; size; dropped; }
class MemoryUsageStore implements BatchUsageStore { record; recordMany; all; size; byUser(id); clear; }
class JsonFileUsageStore implements BatchUsageStore { constructor(path: string); record; recordMany; all; }
...
class SqliteStore implements KeyValueStore { constructor(path: string | { path: string; table?: string }); close(): void; }
class RedisStore implements KeyValueStore { constructor(client: RedisCommands | { client: RedisCommands; prefix?: string }); }
interface RedisCommands { get(key): Promise<string | null>; set(key, value, options?: { NX?: boolean; PX?: number }); del(key | keys); keys(pattern); pttl?(key); }
