...
event, component, scheduled task, prefix command, hybrid command or context menu.
...
scheduled task (`task`), a prefix command (`prefixCommand`), a hybrid command
(`hybridCommand`), or a context-menu command (`userCommand` / `messageCommand`).
Other exports (helpers, constants, types) are ignored. So both of these are picked up:
...
interface LoadOptions {
  ...
  typescript?: boolean;           // default: false — also import .ts / .mts
  recursive?: boolean;            // default: true
}
```

- **`extensions`** — which file extensions to import. By default the loader reads
  `.js`, `.mjs` and `.cjs` — i.e. **compiled JavaScript**.
- **`typescript`** — include `.ts` and `.mts`. Requires a runtime that can load
  TypeScript (`tsx`, `bun`, or Node with type stripping). Otherwise you get a
  clear error instead of a raw `ERR_UNKNOWN_FILE_EXTENSION`.
...
await client.load(`${here}src`, { typescript: true });
```

> Default extensions are compiled JavaScript. **Build first**, then load the
> emitted output — `npx tsc && node dist/index.js`. For a TypeScript source tree
> under `tsx` or `bun`, pass `{ typescript: true }`.
