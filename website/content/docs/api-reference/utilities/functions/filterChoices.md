---
title: "filterChoices()"
description: "Filter a choice list by the user's current autocomplete query."
---

```ts
function filterChoices<V>(
   items: 
  | readonly string[]
  | readonly OptionChoice<V>[], 
   query: string, 
   options?: FilterChoicesOptions): OptionChoice<V>[];
```
