---
title: "Environment & dotenv"
description: "spearkit includes a tiny, dependency-free .env loader and a typed reader over process.env, so a bot needs no extra dotenv dependency. The client auto-loads .env on start(), and…"
---
...
- [Client](/docs/guides/client) — the `dotenv` and other construction options.
- [Logging](/docs/guides/logging) — structured logging that pairs with `env`-driven config.
